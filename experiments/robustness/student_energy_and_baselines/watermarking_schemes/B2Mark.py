...
class B2MarkWatermarkEmbedding:
    def __init__(self, dataset="housing", seed=10000, k=10, g=3, gamma=1/2, secret_key="123", columns_of_interest = ['RM', 'AGE'], watermarked_column = 'MEDV'):
    ...
    def load_data(self, file_path):
        ...
        self.medv_max = self.origin[self.watermarked_column].max()
        self.medv_min = self.origin[self.watermarked_column].min()
    ...
    def process_data(self):
        for idx in range(len(self.origin)):
            ...
            if self.origin.loc[idx, self.watermarked_column] >= self.green_domain_values[closest_idx][0] and \
               self.origin.loc[idx, self.watermarked_column] <= self.green_domain_values[closest_idx][1]:
            ...
            self.origin.loc[idx, self.watermarked_column] = closest_value
...
class B2MarkWatermarkDetection:
    def __init__(self, dataset, secret_key, k=10, g=3, seed=10000, columns_of_interest=['RM', 'AGE'], watermarked_column='MEDV'):
        self.dataset = dataset
        self.secret_key = secret_key
        self.k = k
        self.g = g
    ...
    def detect_watermark(self, watermarked_data):
        ...
        for idx in range(len(watermarked_data)):
            ...
            for low, high in self.green_domains:
                if low <= watermarked_data.loc[idx, self.watermarked_column] < high:
                    green_cell += 1
                    break
