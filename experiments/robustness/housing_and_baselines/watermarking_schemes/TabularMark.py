...
class TabualrMarkWatermarkEmbedding:
    def __init__(self, original_file, n=int(500/3), gamma=1/2, seed=10000, dataset='housing', p = 25, k = 10):
    ...
    def apply_watermark(self):
        ...
        for idx, divide_seed in zip(self.indices, self.divide_seeds):
            ...
            self.origin.loc[idx, 'MEDV'] += perturb_value
...
class TabularMarkWatermarkDetection:
    def __init__(self, origin_file, results_file,watermarked_data=None, n=int(500/3), gamma=1/2, seed=10000, p = 25, k = 10, dataset='housing', primary_key_cols=None):
    ...
    def match_tuples(self, origin_data, watermarked_data, indices):
        ...
        :param watermarked_data: 水印数据
        :param indices: 水印数据中的索引
        :return: 匹配的索引集合
        """
        match_indices = []
    ...
    def detect_watermark(self):
        ...
        for idx, match_idx, divide_seed in zip(self.indices, match_indices, self.divide_seeds):
            ...
            difference = self.watermarked_data.loc[idx, 'MEDV'] - self.origin_data.loc[idx, 'MEDV']
